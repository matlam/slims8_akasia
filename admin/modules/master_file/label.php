...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($labelDesc) OR empty($labelName)) {
        utility::jsAlert(__('Label Name OR Label Description must be filled!'), utility::ALERT_TYPE_ERROR);
        exit();
    } else {
        ...
        if (!empty($_FILES['labelImage']) AND $_FILES['labelImage']['size']) {
            ...
            if ($img_upload_status == UPLOAD_SUCCESS) {
              ...
              utility::jsAlert(__('Label image file successfully uploaded'), utility::ALERT_TYPE_SUCCESS);
            } else {
              // write log
              ...
              utility::jsAlert(__('FAILED to upload label image! Please see System Log for more detailed information'), utility::ALERT_TYPE_ERROR);
            }
        }
