...
if (isset($_POST['doExport'])) {
  ...
  if (empty($_POST['fieldSep']) OR empty($_POST['fieldEnc'])) {
    ...
    if ($dbs->error) {
      utility::jsAlert(__('Error on query to database, Export FAILED!'), utility::ALERT_TYPE_ERROR);
    } else {
        if ($all_data_q->num_rows > 0) {
          ...
          utility::jsAlert(__('There is no record in bibliographic database yet, Export FAILED!', utility::ALERT_TYPE_ERROR));
        }
    }
  }
  exit();
