...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	*/
	...
	$member_q = $dbs->query('SELECT m.member_name, m.member_id, m.member_image, m.birth_date, m.member_address, m.member_email, m.inst_name, m.postal_code, m.pin, m.member_phone, m.expire_date, m.register_date, mt.member_type_name FROM member AS m
        LEFT JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id
        WHERE m.member_id IN('.$member_ids.')');
    $member_datas = array();
    ...
    } else { utility::jsAlert(str_replace('{directory}', SB.FLS, __('ERROR! Cards failed to generate, possibly because {directory} directory is not writable')), utility::ALERT_TYPE_ERROR); }
    exit();
}
